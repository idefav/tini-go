...
type HookManager struct {
	preExecuteHooks map[string]PreExecuteHook
}

func (hm *HookManager) AddPreHook(pe PreExecuteHook) {
	hm.preExecuteHooks[pe.Name()] = pe
}

func (hm *HookManager) GetPreHooks() map[string]PreExecuteHook {
	return hm.preExecuteHooks
}
...
func preExecHook() {
	preHooks := hookMgr.GetPreHooks()
	for name, hook := range preHooks {
		printInfo("Run PreHook: %s", name)
		hook.Exec()
	}
}
...
func init() {
	hookMgr = HookManager{make(map[string]PreExecuteHook)}
	hookMgr.AddPreHook(&hooks.CreateAppEnvHook{})
}
